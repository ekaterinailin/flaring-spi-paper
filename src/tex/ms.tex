...
The second assumption is that $B_{\rm V}$ can be equal to or lower than the maximum radial magnetic field strength. This is because, unlike the maximum radial magnetic field, which is measured at a specific location on the stellar surface, $B_{\rm V}$ may suffer from cancellation as it is measured across a specific area. In this context, the maximum radial magnetic field strength can be considered as the upper limit of $B_{\rm V}$ (15\%~of~B~=~$B_{\rm r}^{\rm max}$). Under these two assumptions, we will be estimating  an upper limit on $\overline{AS}_{R}$.


what we think about it (E and J contributions, comments from others  welcome)
TAP 26 and KOI-12 appear sub-Alfv\'enic in our estimate. So it seems unlikely that the absence of interaction is caused by the planets orbiting outside the Alfv\'en radius. 

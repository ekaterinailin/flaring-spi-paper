...
\begin{abstract}
    In many star-planet systems discovered so far, the innermost planet orbits within only a few stellar radii. In these systems, planets become in-situ probes of the extended stellar magnetic field. Because they disturb the field as they move, they are expected to trigger flares on the stellar surface. These flares are not expected to appear different from intrinsic stellar flares individually.

    However, as we expect planet-induced flares to be associated with the planet's orbital period $P_{orb}$, we can identify their presence from a clustering of flares in phase with the planet's orbital phase. We used the excellent phase coverage obtained from Kepler and the Transiting Exoplanet Survey Satellite to find flaring star-planet systems, compile a catalog of all their flares, and measure how much they cluster in orbital phase. 

    In the 1811 searched systems, we found 25 single stars with three or more flares each. We quantified the significance of the clustering in each system, and compared it against the theoretically expected amplitude of magnetic interaction that leads to planet-induced flaring. Most systems do not show any clustering, as expected from their relatively wide orbits. Those we expect to show planet-induced flaring fall on two branches. An inactive one, without any signs of clustering, and a tentative active one, where the clustering becomes more pronounced as the expected amplitude of interaction increases. The flares in HIP 67522, a young Hot Jupiter system, are prominently clustered ($p<0.006$), identifying it as the most promising candidate for magnetic star-planet interaction in our sample.

  %  In our results, HIP 67522, a 17 Myr old solar type star with a Hot Jupiter in a 7-day orbit, shows the most pronounced clustering with only a 0.006 probability of coincidence. While short of a $3\sigma$ detection,
    
  %  We also searched our data for locally expressed tidal interaction, i.e., flares clustering in phase with the tidal bulges ($P_{\rm orb}/2$). We find a marginal trend with different scaling laws of tidal interaction. Our system-by-system comparison tentatively suggests that young systems can interact both tidally and magnetically, whereas old, magnetically inactive systems mostly interact tidally.
...
\script{paper_illustrative_sketch_of_system_geometries.py}
    \begin{centering}
        \includegraphics[width=\linewidth]{figures/system_geometries.png}
        \caption{
         Simplified magnetic star-planet interaction (SPI) geometries.\textbf{ Panel a: Fully aligned system along the line of sight (LOS). }Magnetic, rotational, and orbital axis are aligned. The interaction footpoint moves along the black line, and is visible to the observer on the red line, i.e. during $50\%$ of the planet's orbit. Flares induced along this line will result in excess flares when the planet is in front of the star. \textbf{Panel b: Orbital misalignment.} Same a panel a., but the planet's orbit is misaligned. The footpoint of interaction is again visible $50\%$ of the time, resulting in modulated flaring with orbital phase. However, it passes different latitudes on the star, which correspond to different dipole field strengths and coronal regions, which may affect the power of interaction. \textbf{Panel c: Inclined fully aligned system.} The red interaction footpoint line is visible $100\%$ of the time. No modulation due to magnetic SPI can be observed, even if flares are triggered by the planet. The sketches only show the interaction on one hemisphere of the star, but they may appear on both.
        }
        \label{fig:sketch}
    \end{centering}
\end{figure*}
...
First, we estimate the power of star-planet interaction generated by the stretch-and-break mechanism using the scaling relations in~\cite{lanza2012starplanet} (their Eqn. 45), where $P_{spi,sb}$ is proportional to

\begin{equation}
    P_{spi,sb} \sim B_p^{2/3} R_p^2 B_*^{4/3} R_*^4 v_{rel} a^{-4} .
\end{equation}
...
\begin{equation}
    P_{spi,sb0} \sim R_p^2 R_*^4 B_*^2 v_{rel}    a^{-4} 
\end{equation}
...
\begin{equation}
    P_{spi,aw} \sim B_p^{2/3} R_{p}^2    B_*^{1/3}  v_{rel}^2 a^{-2}  
\end{equation}
...
\begin{equation}
    P_{spi,aw0} \sim R_p^2  B_*  v_{rel}^2  a^{-4}
\end{equation}

Lacking $B_p$, and various additional factors that appear in $P_{spi,sb},P_{spi,sb0},P_{spi,aw}$, and  $P_{spi,aw0}$, such as the angle between the relative velocity and magnetic field of the stellar wind~\citep{kavanagh2022radio}, or the force-free parameter~\citep{lanza2012starplanet}, we restrict ourselves to differential estimates, and normalize all $P_{spi}$ to AU Mic.
...
\movetableright=-20mm
\script{paper_main_table.py}
\caption{Flaring star-planet interaction. $Ro$, $B$, and $v_{rel}$, are derived from literature values (Table~\ref{tab:maintable_lit}). $P_{spi,xx}$ stands for the power of stretch-and-break ($sb$) and Alfv\'en wing ($aw$) interaction mechanisms, assuming the planet has a magnetic field strength of $1\,$G. $P_{spi,xx0}$ is the same, but assuming an unmagnetized planet. All powers are normalized to AU Mic. The $p$-value of the Anderson-Darling test is lower when the system shows more flares periodic with the planetary orbit.}
...
Although Proxima Cen is part of a triple stellar system, we treat it a single star in this work because of the large angular separation of over 2 deg to its companions, $\alpha$ Cen AB. We do not detect a deviation from intrinsic flaring on Proxima Cen, consistent with numerical models that place Proxima Cen b well outside the sub-Alfv\'enic zone~\citep{gomez2020earthlike, kavanagh2021planetinduced, garraffo2022revisiting}. However, this might not entirely exclude Proxima Cen from the search for flaring star-planet interaction. The tentatively detected Proxima Cen d, a planet further in, at 0.029 au, or 5 day orbital period~\citep{faria2022candidate, artigau2022linebyline} could still cause flaring star-planet interactions. Depending on the phase of the activity cycle, Proxima Cen d may be in the sub-Alf\'enic regime during Proxima Cen's activity maximum~\citep{gomez2020earthlike}. However, the orbital period of the tentative planet is so uncertain at this point that its coherence time~(Section~\ref{sec:results:coherence}) is shorter than the roughly 2yr observing baseline of Proxima Cen, preventing us from accurately measuring the flare phases. 
...
Magnetic star-planet interaction can only take place if the planet spends at least a part of its orbit in the sub-Alfv\'enic regime~(see Section~\ref{sec:intro:mspi}). We estimated the average Alfv\'en surface size ($\overline{AS}_{R}$) using the power law established by \citet{Chebly2023}:

\begin{equation}
    \log \overline{AS_{\rm R}} = (0.44 \pm 0.05) \log B_{\rm r}^{\rm max} + (0.54 \pm 0.08)\label{AS_estimation}
\end{equation}

This relation links the mean AS size with the maximum large-scale radial magnetic field ($B_{\rm r}^{\rm max}$) obtained in Zeeman-Doppler Imaging reconstructions (\citealt{donati2006surprising, morin2008largescale, fares2009magnetic, alvarado-gomez2015activity, hussain2016spectropolarimetric, kochukhov2020mapping}). The relation was derived from 3D~MHD stellar wind simulations cool main sequence stars (F, G, K, and M), which incorporated observed ZDI maps as boundary condition for the stellar radial magnetic field (for more details on the models see \citealt{Chebly2023}). However, the stellar magnetic field~($B$) in this work (Table~\ref{tab:maintable_der}), derived from the Rossby number of each star (Eq.~\ref{Eq3}, \ref{Eq4}), corresponds to the total, unsigned magnetic field strength on the stellar surface. As such, it is necessary to estimate how $B_{\rm r}^{\rm max}$ could be related to $B$. 

In \cite{reiners2022magnetism} it was reported that for Sun-like stars with a total magnetic field strength of up to 2000~G, the longitudinal magnetic field derived from Stokes V spectropolarimetric observations ($B_{\rm V}$) accounts for $\sim$\,$10$\% of the total magnetic field. \cite{kochukhov2020hidden} showed that this trend also holds for M-dwarfs with magnetic fields up to 4000~G. In this case, however, the recovered longitudinal magnetic field accounts for about $20\%$ of the total magnetic field. Moreover, our star sample covers a wide range of magnetic fields, the weakest being 100~G and the strongest $3800\,$G. Therefore, as a first assumption, we consider that the longitudinal magnetic field of our stars would reach 15\% of the total magnetic field (i.e. $B_{\rm V} = 0.15B$), given that most systems in our sample have $B>2000\,$G. 
...
In our results in Fig.~\ref{fig:adtest_bp}, we can interpret the two branches emerging at high expected power of interaction as due to intermittency. TAP 26 and KOI-12 do not show any sign of excess flaring in our data, despite similar expected powers as in HIP 67522 with its $>2.5\sigma$ signal. However, other explanations cannot be ruled out yet (see Sections~\ref{sec:results:individualstars:tap26} and \ref{sec:results:individualstars:koi12}, and the following).
...
\begin{figure*}[t]
    ...
    \caption{Deviation from random flare times in phase with $P_{\rm orb}/2$, i.e., in phase with tidal bulges raised on the star, compared to different models of tidal interaction~(Section~\ref{sec:discussion:tidal}). \textbf{Cross symbols} mark planets with $M_p \sin i$ measurements. \textbf{Round symbols} mark planets with measured  or estimated $M_p$ (see Section~\ref{sec:discussion:mpms}). 
